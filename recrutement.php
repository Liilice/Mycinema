<?php
    ...
    $statement = $pdo->prepare('SELECT salary FROM job');
    $statement->execute();
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $_POST = filter_input_array(INPUT_POST, [
            ...
            'job' => FILTER_SANITIZE_FULL_SPECIAL_CHARS,
            'contrat' => FILTER_SANITIZE_FULL_SPECIAL_CHARS,
        ]);
        $nom = $_POST["nom"];
        $prenom = $_POST["prenom"];
        $email = $_POST["email"];
        ...
        $job = $_POST["job"] ;
        $contrat = $_POST["contrat"]."T00:00:00";
        if (!$nom)  {
        ...
        if (!$contrat)  {
            $error['contrat'] = ERROR_REQUIRED; 
        }
        if ($job === "defaut")  {
            $error['job'] = ERROR_REQUIRED; 
        }
        
        if(!empty($nom)&&!empty($prenom)&&!empty($email)&&!empty($birthdate)&&!empty($address)&&!empty($zipcode)&&!empty($city)&&!empty($country)&&!empty($job)&&!empty($contrat)){
            ...
            $statementUserId = $pdo->query("SELECT id FROM user WHERE email LIKE '$email';");
            $resultatUserId = $statementUserId->fetchAll(PDO::FETCH_ASSOC);
            $idUserAdd = $resultatUserId[0]["id"];

            $statementIdJob = $pdo->prepare("SELECT * FROM job WHERE salary = $job;");
            $statementIdJob->execute();
            $resultatIdJob = $statementIdJob->fetchAll();
            $idJob = $resultatIdJob[0]["id"];
            
            $statementEmployee = $pdo->query("INSERT INTO employee(id_user, id_job, date_begin) VALUES($idUserAdd, $idJob, '$contrat');");
...
<body>
    <form action="" method="POST" >
        ...
        <div class="labelInput">
            <label for="job">Postuler pour un salaire de : </label>
        ...
        <div class="labelInput">
            <label for="contrat">Date de contrat</label>
            <input type="date" name="contrat">
            <?= $error['contrat'] ? '<p class="textError">' . $error['contrat'] . '</p>' : "" ?>
        </div>
